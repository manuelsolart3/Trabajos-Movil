...
data class ManangerBd(val context: Context) { //creamos una instancia
    ...
    fun getData():ArrayList<Ciudad>{
        ...
        val  cursor = bd.rawQuery(constantes.DATA,null))

        if (cursor.moveToFirst()){
            //se verifica si hay datos en la primera poscicion del cursor
            do{
                // se alamacenan las variables lo que tiene el cursor en la columna 1
